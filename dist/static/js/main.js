...
document.addEventListener("DOMContentLoaded", function () {
  ...
  */
  ...
  ChiefSlider.prototype._refresh = function () {
    ...
    }.bind(this)); // скрытие кнопок слайдера
  ...
  var smoothHeight = function smoothHeight(itemSelector, buttonSelector, contentSelector) {
    ...
    var items = document.querySelectorAll(itemSelector); // находим все элементы по переданному селектору в параметре itemSelector и записываем в константу items

    if (!items.length) return; // если таких элементов нет, прекращаем выполнение функции

    items.forEach(function (el) {
      // для каждого элемента
      var button = el.querySelector(buttonSelector); // находим кнопку и записываем в константу button

      var content = el.querySelector(contentSelector); // находим блок с контентом и записываем в константу content
      ...
      button.addEventListener('click', function () {
        ...
        if (el.dataset.open !== 'true') {
          ...
          content.style.maxHeight = "".concat(content.scrollHeight, "px"); // и блоку с контентом устанавливаем inline-свойсво max-height со вычисленным значением полной высоты этого блока
        } else {
      ...
      var onResize = function onResize() {
        ...
        if (el.dataset.open === 'true') {
          ...
          if (parseInt(content.style.maxHeight) !== content.scrollHeight) {
            ...
            content.style.maxHeight = "".concat(content.scrollHeight, "px"); // только тогда блоку с контентом корректируем значение inline-свойсва max-height
          }
        }
      };
